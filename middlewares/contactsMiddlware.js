...
exports.validateId = catchAuthErr(async (req, res, next) => {
  ...
  if (valideId) return next(new ErrorId(404, 'User don`t found'))
  const contacts = await User.exists({ _id: id })
  if (contacts) return next(new ErrorId(404, 'User don`t found'))
  next()
})
...
exports.dataConflict = catchAuthErr(async (req, res, next) => {
  ...
  if (error)
    return next(
      new AppErr(400, 'Error from joi or different`s validation library'),
    )
...
exports.validateToken = catchAuthErr(async (req, res, next) => {
  ...
  if (!currentUser) return next(new AppErr(401, 'Don`t logIn to account'))
...
exports.allowFor = (...role) => async (req, res, next) => {
  ...
  next(new AppError(403, 'You`re forbidden, see next recources'))
}
